...
- Admins can also define HTTPS/API providers (Zoho Mail API, ZeptoMail API, or AWS SES HTTPS) with per-minute/per-day quotas; batches are rotated across this pool before falling back to the legacy SMTP rotation when no API providers are enabled.
...
- Persistent config lives in `data/mail-providers.json`. Each provider has `type` (`smtp`, `ses`, `zoho`, or `zepto`), credentials, and quota caps. Quotas are tracked by day + rolling one-minute windows.
...
- When at least one provider is enabled, every job batch is routed through the provider pool (Zoho form-post, ZeptoMail HTTPS, SES HTTPS, or raw SMTP). If all providers are disabled or missing, the system falls back to the legacy SMTP rotation controlled by `data/smtp-pool.json`.
...
#### ZeptoMail shortcut
- The Zepto provider type uses the ZeptoMail HTTPS API (`Authorization: Zoho-enczapikey …`) so you don’t need to stand up SMTP credentials.
- Configure it manually via the Admin UI (type = ZeptoMail) or set the following `.env` aliases to auto-create a provider on startup:
  ```
  APP_ZEPTO_KEY=enc:<base64 api key>
  APP_ZEPTO_FROM=no.reply@yourdomain.com
  APP_ZEPTO_URL=https://api.zeptomail.com/v1.1/as/email   # optional
  APP_ZEPTO_BOUNCE=bounce@yourdomain.com                  # optional
  APP_ZEPTO_REPLY=support@yourdomain.com                  # optional
  APP_ZEPTO_QPM=60                                        # optional quota per minute
  APP_ZEPTO_QPD=1000                                      # optional quota per day
  ```
  Any `enc:` prefix is decoded automatically by the server before storing the provider, so secrets never appear in cleartext files (all JSON data is stored obfuscated on disk).
